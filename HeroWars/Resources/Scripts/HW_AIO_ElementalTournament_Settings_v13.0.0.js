...
(function (global) {
    ...
    function createSettingsModal(options = {}) {
        ...
        tempSettings = {
            ...
            minRivalPower: currentSettings.minRivalPower !== undefined ? currentSettings.minRivalPower : DEFAULT_SETTINGS.minRivalPower
        };
        ...
        soundCheck.onchange = function () {
            tempSettings.soundAlertEnabled = this.checked;
        };
        ...
        // ========== ПОЛЕ: МИНИМАЛЬНАЯ МОЩЬ СОПЕРНИКОВ ==========
        const minPowerRow = document.createElement('div');
        minPowerRow.className = 'settings-field';

        const minPowerLabelContainer = document.createElement('div');
        minPowerLabelContainer.style.cssText = 'display: flex; align-items: center;';

        const minPowerLabel = document.createElement('span');
        minPowerLabel.className = 'label';
        minPowerLabel.textContent = 'Минимальная мощь соперников (в тыс.):';

        minPowerLabelContainer.appendChild(minPowerLabel);

        const minPowerInput = document.createElement('input');
        minPowerInput.type = 'number';
        minPowerInput.min = '0';
        minPowerInput.max = '99999';
        minPowerInput.placeholder = '700';
        minPowerInput.value = tempSettings.minRivalPower;
        minPowerInput.onchange = function () {
            const val = parseInt(this.value);
            if (!isNaN(val) && val >= 0) {
                tempSettings.minRivalPower = val;
            } else {
                this.value = tempSettings.minRivalPower;
            }
        };

        minPowerRow.appendChild(minPowerLabelContainer);
        minPowerRow.appendChild(minPowerInput);
        fieldsContainer.appendChild(minPowerRow);
        ...
        // ========== ИНФОРМАЦИОННЫЙ БЛОК (СВЁРНУТЫЙ, С КНОПКОЙ "?") ==========
        const infoBlock = document.createElement('div');
        infoBlock.className = 'info-block';

        // Заголовок с кнопкой "?" — вся строка кликабельна
        ...
        const infoHeaderLeft = document.createElement('div');
        infoHeaderLeft.style.cssText = 'display: flex; align-items: center; gap: 8px;';

        const infoQuestionBtn = document.createElement('span');
        infoQuestionBtn.textContent = '?';
        infoQuestionBtn.style.cssText = [
            'display: inline-flex',
            'align-items: center',
        ...
        infoTitle.textContent = 'Как работает проверка';
        infoTitle.style.cssText = 'color: #e8e8e8; font-weight: 600; font-size: 13px; transition: color 0.2s ease;';

        infoHeaderLeft.appendChild(infoQuestionBtn);
        infoHeaderLeft.appendChild(infoTitle);

        const infoToggleArrow = document.createElement('span');
        infoToggleArrow.textContent = '▼';
        infoToggleArrow.style.cssText = 'color: #667eea; font-size: 12px; transition: transform 0.3s ease;';

        infoHeader.appendChild(infoHeaderLeft);
        infoHeader.appendChild(infoToggleArrow);

        // Скрытый контент
        const infoContent = document.createElement('div');
        infoContent.style.cssText = 'padding-left: 28px; line-height: 1.8; color: #d0d0d0; display: none; margin-top: 8px; font-size: 13px;';
        ...
        function toggleInfo() {
            ...
            infoToggleArrow.style.transform = isHidden ? 'rotate(180deg)' : 'rotate(0deg)';
            infoQuestionBtn.style.background = isHidden ? '#4CAF50' : '#667eea';
            infoQuestionBtn.textContent = isHidden ? '✓' : '?';
        }

        // Клик по всей верхней строке
        infoHeader.onclick = toggleInfo;

        // Ховер на всю строку (для десктопа)
        infoHeader.onmouseenter = function () {
            infoQuestionBtn.style.transform = 'scale(1.15)';
            infoTitle.style.color = '#fff';
        };
        infoHeader.onmouseleave = function () {
            infoQuestionBtn.style.transform = 'scale(1)';
            infoTitle.style.color = '#e8e8e8';
        };
        ...
        resetBtn.onclick = function () {
            ...
            tempSettings = {
                ...
                minRivalPower: DEFAULT_SETTINGS.minRivalPower
            };
        ...
        saveBtn.onclick = function () {
            const finalSettings = {
                ...
                minRivalPower: tempSettings.minRivalPower
            };
